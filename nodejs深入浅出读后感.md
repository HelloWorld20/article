...
自定义模块则默认从当前node_modules查找，找不到则到上层目录查找，一直查找到跟目录的node_modlues。查找路径可由`console.log(modkue.paths)`看到

对于文件扩展名顺序是: .js、.json、.node一次补足扩展名尝试加载

如果再找不到，则会在当前目录下以一下顺序再查找：
